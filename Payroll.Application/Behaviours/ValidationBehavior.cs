    ...
    where TRequest : IRequest<TResponse>
{
    ...
    {
        _validators = validators; // All validators for this request injected by DI
    }
        ...
        RequestHandlerDelegate<TResponse> next)
    {
        if (_validators.Any())
        {
            ...
            if (failures.Count != 0)
            {
                // Return failure using ApiResponse if TResponse is ApiResponse<T>
                ...
                if (responseType.IsGenericType && responseType.GetGenericTypeDefinition() == typeof(ApiResponse<>))
                {
                    ...
                    return (TResponse)failMethod!.Invoke(null, new object[] { string.Join(", ", failures.Select(f => f.ErrorMessage)) })!;
                }
